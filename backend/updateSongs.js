...
// Conexão com MongoDB
mongoose.connect(MONGODB_URI, { dbName: MONGODB_DB_NAME// Use a string de conexão Mongoose
        });

// Importar o array de músicas
// Remova os comentários do seu array real ou importe do arquivo onde está definido
const songsArray = [
    {
...
// Schema do artista (deve corresponder ao seu esquema existente)
...
// Função para fazer upload das músicas com referências corretas
async function uploadSongsWithArtistRefs() {
  try {
    // 1. Extrair nomes únicos de artistas do array
    const uniqueArtistNames = [...new Set(songsArray.map(song => song.artist))];
    console.log(`Encontrados ${uniqueArtistNames.length} artistas únicos no array.`);

    // 2. Buscar os artistas do banco de dados
    const artistMap = {};
    for (const artistName of uniqueArtistNames) {
      const artist = await Artist.findOne({ name: artistName });
      
      if (artist) {
        artistMap[artistName] = artist._id;
        console.log(`Artista encontrado: ${artistName}`);
      } else {
        console.log(`Aviso: Artista "${artistName}" não encontrado no banco de dados.`);
        // Você pode escolher criar o artista aqui se quiser:
        // const newArtist = await Artist.create({ name: artistName });
        // artistMap[artistName] = newArtist._id;
      }
    }

    // 3. Transformar as músicas para incluir referências de ObjectId
    const songsToUpload = [];
    
    for (const song of songsArray) {
      if (artistMap[song.artist]) {
        // Criar objeto da música com a referência ao artista
        songsToUpload.push({
          image: song.image,
          name: song.name,
          duration: song.duration,
          artist: artistMap[song.artist], // ObjectId do artista
          audio: song.audio
        });
      } else {
        console.log(`Pulando música "${song.name}" porque o artista "${song.artist}" não foi encontrado.`);
      }
    }

    // 4. Fazer upload das músicas para o banco de dados
    if (songsToUpload.length > 0) {
      const result = await Song.insertMany(songsToUpload);
      console.log(`Sucesso! ${result.length} músicas foram adicionadas ao banco de dados.`);
    } else {
      console.log('Nenhuma música foi adicionada ao banco de dados.');
    }
